...
{
    switch (role) {
        case WheelRole::FRONT_LEFT:  return "FL";
        case WheelRole::FRONT_RIGHT: return "FR";
        case WheelRole::REAR_LEFT:   return "RL";
        case WheelRole::REAR_RIGHT:  return "RR";
...
void DeviceConfigManager::unlock()
{
    xSemaphoreGive(m_mutex);
}

uint16_t DeviceConfigManager::getDeviceId() const
{
...
{
    if (m_flash == nullptr) return false;

    DeviceConfig cfg;
    m_flash->read(CONFIG_FLASH_ADDR, reinterpret_cast<uint8_t*>(&cfg), sizeof(cfg));
...
bool DeviceConfigManager::saveToFlash()
{
    ...
    // Verify write
    DeviceConfig verify;
    m_flash->read(CONFIG_FLASH_ADDR, reinterpret_cast<uint8_t*>(&verify), sizeof(verify));
