 ...
 * and filled circle. All functions take LCD& as first parameter.
 *
 ...
 */
inline void fillTriangle(LCD& lcd,
    ...
    uint16_t color)
{
    ...
    if (y0 == y2) {
        ...
        // Clip to screen
        if (minX < 0) minX = 0;
        if (maxX >= LCD::WIDTH) maxX = static_cast<int16_t>(LCD::WIDTH - 1);
        if (minX <= maxX && y0 >= 0 && y0 < LCD::HEIGHT) {
    ...
    for (int16_t y = y0; y <= y2; y++) {
        ...
        // Clip to screen
        if (xa < 0) xa = 0;
        if (xb >= LCD::WIDTH) xb = static_cast<int16_t>(LCD::WIDTH - 1);
        if (xa > xb) continue;
 ...
 */
inline void drawCircleOutline(LCD& lcd,
 ...
 */
inline void fillCircle(LCD& lcd,
    int16_t cx, int16_t cy, int16_t r,
    uint16_t color)
{
    if (r <= 0) {
