...
bool FlashImageService::init(Harness::IFlash* flash) {
    if (flash == nullptr) {
        return false;
    }
...
bool FlashImageService::eraseSlot(uint32_t slot) {
    ...
    ITRACE(Harness::ITrace::L4_FLASH, "[L3>L4]", "flash.erase", "slot");
...
bool FlashImageService::writeSlotData(uint32_t slot, uint32_t offset,
    ...
    ITRACE(Harness::ITrace::L4_FLASH, "[L3>L4]", "flash.write", "slot");
...
bool FlashImageService::readSlotChunk(uint32_t slot, uint32_t offset,
    ...
    ITRACE(Harness::ITrace::L4_FLASH, "[L3>L4]", "flash.read", "slot");
    uint32_t addr = slotAddress(slot) + offset;
...
bool FlashImageService::eraseAll() {
    ...
    ITRACE(Harness::ITrace::L4_FLASH, "[L3>L4]", "flash.erase", "all");
