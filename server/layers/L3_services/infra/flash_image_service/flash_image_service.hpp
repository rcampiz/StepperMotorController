...
namespace Harness { class IFlash; }
...
// ── Service class ───────────────────────────────────────────────────
class FlashImageService : public Harness::IFlashImageAccess {
public:
     ...
     */
    bool init(Harness::IFlash* flash);

    /** @brief Is the service ready? */
    bool isAvailable() const override { return m_flash != nullptr && m_maxSlots > 0; }

    /** @brief Get flash info snapshot. */
    FlashImageInfo getInfo() const;

    /** @brief Maximum image slots this flash can hold. */
    uint32_t maxSlots() const override { return m_maxSlots; }
     ...
     */
    bool eraseSlot(uint32_t slot) override;
...
private:
    Harness::IFlash* m_flash       = nullptr;
